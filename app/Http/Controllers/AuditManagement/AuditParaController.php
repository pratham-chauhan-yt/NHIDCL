...
class AuditParaController extends Controller
{
    ...
    public function storeAuditPara(Request $request)
    {
        try {
            ...
            $inputs = $request->all();
            $auditPara =  AuditQueryPara::create([
            ...
            if ($assignedUser) {
                ...
                // Encrypt the ID
                $encryptedId = Crypt::encrypt($auditId);

                // Generate route with encrypted ID
                $link = route('audit-management.view', $encryptedId);

                $assignedUser->notify(new UserNotification($message, $link));
            }
            // $financeUsers = User::role('AMS Candidate')->get(); 
            // foreach ($financeUsers as $user) {
            //     $user->notify(new UserNotification('📌 CC: ' . $auditPara->title, $link));
            // }
    ...
    public function updateAuditPara(Request $request)
{
    try {
        $validator = Validator::make($request->all(), [
            'para_id'         => 'required|integer',
            'audit_para_year' => 'required',
            'para_letter_no'  => 'required',
            'title'           => 'required',
            'brief_para'      => 'required',
        ...
        $para = AuditQueryPara::find($request->para_id);

        if (!$para) {
            Alert::error('Error', 'Invalid Para ID!');
            return redirect()->back();
        }

        // Update only text fields
        $para->year                     = $request->audit_para_year;
        $para->nhidcl_ams_audit_query_id = $request->para_letter_no;
        $para->title                    = $request->title;
        $para->brief_para               = $request->brief_para;
        $para->ref_query_type_id        = $request->query_type;
        $para->ref_part_id              = $request->ref_part_id;
        $para->ref_office_id            = $request->office;
        $para->ref_department_id        = $request->department;
        $para->assign_to                = $request->assign_to;
        $para->updated_by               = user_id();

        // Store file path from hidden input (same as store)
        $para->pdf_file_path  = $request->para_pdf_file;
        $para->word_file_path = $request->para_word_file;

        $para->save();

        Alert::success('Success', 'Audit Query Para updated successfully.');
        return redirect()->back();

    ...
    if (year) {
        ...
        $.ajax({
            ...
            success: function (data) {
                $letterNo.empty().append('<option value="">Select Letter No.</option>');
...
$(document).on("change", ".upload_para_pdf, .upload_para_word", function () {
    const websiteMeta = document.querySelector('meta[name="website-url"]');
    const websiteUrl = websiteMeta?.getAttribute("content");
    const finalUrl = websiteUrl ? `${websiteUrl}/audit-management/upload/para` : null;
...
$(document).on('change', 'select[name="department"]', function () {
    ...
    if (departmentId) {
        $.ajax({
            ...
            success: function (data) {
                $assignTo.empty().append('<option value="">Select Candidate</option>');
            ...
            error: function () {
                $assignTo.html('<option value="">Failed to load users</option>');
            }
        });
    } else {

...
function personalDeatils(response) {
    ...
    const files = {
        upload_photoss: ["remove_profile_photo", "upload_photos", "upload_photos_filepath", ".attachment_section_photos", "temp_photos", "hide_upload_photos"],
        upload_signaturee: ["remove_profile_signature", "upload_signature", "upload_signature_filepath", ".attachment_section_sign", "temp_signature", "hide_upload_signature"],
        upload_caste_certificatee: ["remove_caste_certificate", "upload_caste_certificate", "upload_caste_certificate_filepath", ".attachment_section_caste_certificate", "temp_caste", "hide_upload_caste_certificate"],
        upload_disability_prooff: ["remove_disability_proof", "upload_disability_proof", "upload_disability_proof_filepath", ".attachment_section_disability_proof", "temp_disability", "hide_upload_disability_proof"],
        upload_dob_prooff: ["remove_dob_proof", "upload_dob_proof", "upload_dob_proof_filepath", ".attachment_section_dob_proof", "temp_dob", "hide_upload_dob_proof"],
        upload_ex_serviceman_prooff: ["remove_ex_serviceman_proof", "upload_ex_serviceman_proof", "upload_ex_serviceman_proof_filepath", ".attachment_section_ex_serviceman_proof", "temp_ex_serviceman", "hide_upload_ex_serviceman_proof"],
        upload_identity: ["remove_identity_proof", "upload_identity_proof", "upload_identity_proof_filepath", ".attachment_section_upload_identity", "temp_identity", "hide_upload_identity_proof"],
    };

    for (const [inputId, [uploadFileClass, fileKey, pathKey, sectionClass, tempId, hideUploadBtnClass]] of Object.entries(files)) {
        const file = data[fileKey];
        const path = data[pathKey];
        const btnname = uploadFileClass;
        ...
        if (file) {
            $(sectionClass).append(`
                <div id="${tempId}" class="my-3">
                    ...
                    <a href="javascript:void(0);" class="bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 ${btnname}" data-id="2">Remove</a>
                </div>`);
...
$(document).on('click', '.remove_profile_photo', function() {
    $(this).parent().hide();
    $(this).closest('.attachment_preview').find('.upload_cust').show();
    $(this).closest('.attachment_preview').find('input').val('');
});

$(document).on('click', '.remove_profile_signature', function() {
    $(this).parent().hide();
    $(this).closest('.attachment_preview').find('.upload_cust').show();
    $(this).closest('.attachment_preview').find('input').val('');
});

$(document).on('click', '.remove_dob_proof', function() {
    $(this).parent().hide();
    $(this).closest('.attachment_preview').find('.upload_cust').show();

...
<div class="inner_page_dash__">
    <div class="my-4">
        ...
        <div id="Home" class="tabcontent" style="display: block;">
            <form action="{{ route('employee-management.exit.interview.update', Crypt::encrypt($interview->id ?? 0)) }}" method="POST" class="form_grid_cust">
                @csrf
                @method('PUT')
                ...
                <div class="form-input">
                    ...
                    <select name="ref_checker_id" id="ref_checker_id" required>
                        ...
                        <option value="{{$managerdata->id}}" {{ $interview->ref_checker_id == $managerdata->id ? 'selected' : '' }}>{{$managerdata->name."  ".$managerdata->email}}</option>
                        @empty
                        @endforelse
                    </select>
                </div>
                <div class="form-input">
                    ...
                    <select name="ref_approver_id" id="ref_approver_id" required>
                        ...
                        <option value="{{$gmanagerdata->id}}" {{ $interview->ref_approver_id == $gmanagerdata->id ? 'selected' : '' }}>{{$gmanagerdata->name."  ".$gmanagerdata->email}}</option>
                        @empty
                        @endforelse
                    </select>
                </div>
                <div class="form-input">
                    ...
                    <textarea name="reason" id="reason" rows="3" placeholder="Enter exit interview reason" required>{{ old('reason', $interview->reason ?? '') }}</textarea>
            ...
            <div class="button_flex_cust_form">
                <button type="submit" class="hover-effect-btn fill_btn" name="submit" value="Submit">Update</button>
            </div>

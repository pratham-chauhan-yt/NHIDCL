        ...
        <div class="my-4 ">
            ...
            <div id="Home" class="tabcontent" style="display: block;">
                ...
                <form class="form_grid_cust" id="add-bg" enctype="multipart/form-data"
                    action="{{ route('bgms.bg.store') }}" method="POST">
                    @csrf
                    <div class="inpus_cust_cs form_grid_dashboard_cust_">
                            ...
                            <select name="nhidcl_bgm_project_details_id" class="">
                                ...
                                @foreach (NhidclProject::where('ref_project_state_id', bgmsAssignedState())->get() as $project)
                                    <option value="{{ $project->id }}"
                                        {{ old('nhidcl_bgm_project_details_id') == $project->id ? 'selected' : '' }}>
                        ...
                        <div class="">
                            ...
                            <input type="text" name="bg_no" placeholder="Enter BG number" value="{{ old('bg_no') }}">
                            @error('bg_no')
                                <div class="error-message">{{ $message }}</div>
                            @enderror
                        </div>
...
<!-- <div class="">
    ...
    <select name="claim_lodged" class="">
        ...
        <option value="TRUE" {{ old('claim_lodged') === 'TRUE' ? 'selected' : '' }}>TRUE</option>
        <option value="FALSE" {{ old('claim_lodged') === 'FALSE' ? 'selected' : '' }}>FALSE</option>
    </select>
    @error('claim_lodged')
        <div class="error-message">{{ $message }}</div>
    @enderror
                        ...
                        <div class="attachment_section_upload_attachment attachment_preview">
                            ...
                            <div class="flex gap-[10px]">
                                ...
                                <label class="upload_cust mb-0 hover-effect-btn"> Upload Filed
                                    ...
                                    <input type="hidden" id="bg_file" name="bg_file" value="{{ old('bg_file') }}">
                                </label>
                            </div>
                            @error('bg_file')
                                <div class="error-message">{{ $message }}</div>

...
class QmsQueryTypeController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $getData = QmsRefQueryType::orderBy('query_type', 'desc')->get();

            return DataTables::of($getData)
                ->addIndexColumn()
                ->addColumn('action', function ($row) {
                    $deleteUrl = route('master-settings.qms-query-type.destroy', Crypt::encrypt($row->id));
        ...
        $header = true;
        $sidebar = true;
        return view("master-settings.qmsQueryType", compact("header", "sidebar"));
    }
    ...
    public function store(Request $request)
    {
        try {
            $validated = $request->validate([
                'query_type' => 'required|max:50',
            ]);
            $saveData = QmsRefQueryType::create($validated);
            ...
            return redirect()->route('master-settings.qms-query-type.index')->withInput()->withErrors(['msg' => $e->getMessage()]);
        }
    }
    ...
    {
        try {
            $verifyId = QmsRefQueryType::where('id', Crypt::decrypt($id))->first();
            if ($verifyId) {
    ...
    public function update(Request $request, string $id)
    {
        try {
            ...
            QmsRefQueryType::where('id', Crypt::decrypt($id))->update($updateArray);
    ...
    {
        try {
            $auditType = QmsRefQueryType::find(Crypt::decrypt($id));

            if (!$auditType) {
                Alert::error('Error', 'Something went wrong, Please try again.');
                return redirect()->route('master-settings.qms-query-type.index')->withErrors(['msg' => 'Something went wrong, Please try again.']);
            }
            $auditType->update(['updated_by' => Auth::user()->id]);
            $auditType->delete();

            Alert::success('Success', 'Audit Type deleted successfully');
            return redirect()->route('master-settings.qms-query-type.index')->with('success', 'Audit Type deleted successfully');
        } catch (\Exception $e) {
            Alert::error('Error', 'Oops, something went wrong. Please try again later.');
            return redirect()->route('master-settings.qms-query-type.index')->withErrors(['msg' => $e->getMessage()]);
        }
    }
}

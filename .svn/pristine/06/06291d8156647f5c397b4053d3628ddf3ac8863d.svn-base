$(document).ready(function () {
    ...
    function toggleFields() {
        ...
        // Show all fields and make required
        $.each(fields, (_, field) => {
    ...
    initValidation("#edit-document-form", {
        ...
        ref_type_id: {
            required: function () { return $("#field_type").is(":visible"); }
        },
        ref_department_id: {
            required: function () { return $("#field_department").is(":visible"); }
        },
        year: {
            required: function () { return $("#field_year").is(":visible"); }
        },
        upload_doc: {
            required: function () { return !$("#upload_doc_url").val(); }
        },
        upload_doc_url: {
            required: function () { return !$("#upload_doc_url").val(); }
        }
        ...
        upload_doc: {
            required: "Please upload a document (if none already uploaded)"
        },
        upload_doc_url: {
            required: "Please upload a document (if none already uploaded)"
        }
    ...
    // Init file upload (same config as add)
    initFileUpload({

$(document).ready(function() {
    ...
    let finalUrl = websiteUrl ? `${websiteUrl}/list-of-candidate-for-chairperson` : null;
        ...
        ajax: {
            ...
            dataSrc: function(json) {
                if (json.recordsTotal === 0) {
                    $('#candidateTable').DataTable().settings()[0].oLanguage.sEmptyTable =
                        `<div class='text-center text-gray-600 font-semibold'>${json.customMessage || "No data found."}</div>`;
                }
                return json.data;
            }
        },
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    ...
                    data.forEach((member, index) => {
                        membersHTML += `
                            <button class="p-0 m-0 px-1" data-popover-target="popover-${row.id}-${index}" data-popover-trigger="hover" type="button">
                                <span class="${member.status === "Selected" ? "selected" : "rejected"}">${member.status}</span>
                            </button>
                            <div data-popover id="popover-${row.id}-${index}"
                                role="tooltip"
                                ...
                                <div class="bg-gray-400 border-b border-gray-600 rounded dark:border-gray-800 dark:bg-gray-900">
                                    <h3 class="font-semibold ps-1">${member.name} (${member.email})</h3>
                                </div>
                                <div class="py-2">
                                    <p>${member.remark}</p>
                                </div>
    ...
    function initializePopovers() {
        ...
        $('[data-popover]').each(function() {
            ...
            const triggerEl = $(this).prev('[data-popover-target]')[
            0]; // The trigger element (button)
            const options = {
                placement: 'bottom',
                triggerType: 'hover',
                offset: 5,
    ...
    $('#requisitionId').on('change', function() {
        ...
        if (requisitionId) {
            let finalUrl = websiteUrl ? `${websiteUrl}/resource-pool/chairperson/selection-process` : null;
            $.ajax({
                url: finalUrl,
                type: "GET",
                data: {
    ...
    $('#saveDraftBtn').on('click', function() {
        ...
        let finalUrl = websiteUrl ? `${websiteUrl}/save-draft-shortlist-for-chairperson` : null;
    ...
    $('#generateShortlist').on('click', function() {
        ...
        Swal.fire({
            ...
            let finalUrl = websiteUrl ? `${websiteUrl}/generate-shortlist-for-chairperson` : null;

    ...
    <div class="inner_page_dash__">
        <div class="my-4 ">
            <div class="tab_custom_c">
                ...
                <button class="tablink" onclick="openPage('EditPara', this, '#373737')">
            ...
            <div id="Home" class="tabcontent">
                ...
                <form class="form_grid_cust" action="{{ route('audit-management.store') }}" method="post"
                    enctype="multipart/form-data" id="edit-auditQuery">
                    @csrf
                        ...
                        <div class="">
                            ...
                            <div class="grid grid-cols-2 gap-[10px]">
                                ...
                                <div class="flex flex-col">
                                    <input type="date" name="from_date" value="{{ $auditQuery->from_date }}">
                                    @error('from_date')
                                ...
                                <!-- TO DATE -->
                                <div class="flex flex-col">
                                    <input type="date" name="to_date" value="{{ $auditQuery->to_date }}">
                                    @error('to_date')
                                    <div class="error-message text-red-600 text-sm mt-1">{{ $message }}</div>
                                    @enderror
                                </div>
            ...
            <div id="EditPara" class="tabcontent">
                <div class="parrent_dahboard_ chart_c inner_body_style inner_pages mt-0">
                    <div class="">Edit Para</div>
                </div>

                <form class="form_grid_cust" action="{{ route('audit-management.update-audit-para') }}" method="post"
                    ...
                    <div class="inpus_cust_cs form_grid_dashboard_cust_">
                        ...
                        <div class="">
                            <label class="required-label">Audit Year</label>
                            <!-- <select class="" name="audit_para_year" id="audit_para_year">
                        ...
                        <div class="">
                            <label class="required-label">Letter No.</label>
                            <select class="" name="para_letter_no" id="para_letter_no">
                                <option value="">Select Letter No.</option>

                                {{-- Pre-fill old selected Letter No --}}
                                @if(!empty($auditPara->nhidcl_ams_audit_query_id) && isset($auditQuery))
                                <option value="{{ $auditQuery->id }}" selected>
                                    {{ $auditQuery->letter_no }}
                                </option>
                                @endif
                            </select>
                        </div>
                        ...
                        <div class="">
                            <label class="">Title</label>
                            <input type="text" name="title" value="{{ $auditPara->title }}">
                        </div>

                        <div class="">
                            <label class="">Brief Report/Para</label>
                            <textarea rows="1" name="brief_para">{{ $auditPara->brief_para }}</textarea>
                        </div>
                        ...
                        <div class="">
                            ...
                            <select name="query_type">
                                ...
                                <option value="1" {{ $auditPara->ref_query_type_id == 1 ? 'selected' : '' }}>
                                    Audit Observation
                                </option>

                                <option value="2" {{ $auditPara->ref_query_type_id == 2 ? 'selected' : '' }}>
                                    Audit Requisition
                                </option>

                                <option value="3" {{ $auditPara->ref_query_type_id == 3 ? 'selected' : '' }}>
                                    Half Margin
                                </option>
                            </select>
                        </div>
                        ...
                        <div class="">
                            <label class="">Part</label>
                            <select class="" name="ref_part_id">
                                <option value="">Select Part</option>
                                @foreach ($parts as $part)
                                <option value="{{ $part->id }}"
                                    {{ $auditPara->ref_part_id == $part->id ? 'selected' : '' }}>
                                    {{ $part->part }}
                                </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="attachment_section_upload_para_pdf">
                            <label>Upload PDF File (<span style="font-size: 10px;">Max 1MB & file should be PDF</span>)</label>

                            <div class="flex gap-[10px]">
                                {{-- Show Old File Name --}}
                                <input type="text"
                                    id="uploaded_para_pdf"
                                    name="uploaded_para_pdf"
                                    placeholder="Upload PDF"
                                    class="uploaded_para_pdf"
                                    readonly
                                    value="{{ $auditPara->pdf_file_path ?? '' }}">

                                <label class="upload_cust mb-0 hover-effect-btn"> Upload File
                                    <input type="file" id="upload_para_pdf" name="upload_para_pdf" class="hidden upload_para_pdf">
                                    <input type="hidden" id="para_pdf_file" name="para_pdf_file"
                                        value="{{ $auditPara->pdf_file_path ?? '' }}">
                                </label>
                            </div>

                            {{-- View Existing PDF File --}}
                            @if (!empty($auditPara->pdf_file_path))
                            @php
                            $filePath = 'uploads/audit-management/para/';
                            $fileName = basename($auditPara->pdf_file_path);
                            $fileUrl = url('audit-management/view/files') .
                            '?pathName=' . $filePath .
                            '&fileName=' . $fileName;
                            @endphp

                            <a href="{{ $fileUrl }}" target="_blank" class="text-primary underline" title="View PDF file">
                                View File
                            </a>
                            @endif

                            @error('upload_para_pdf')
                            <div class="error-message">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        {{-- WORD FILE SECTION --}}
                        <div class="attachment_section_upload_para_word">
                            <label>Upload Word File (<span style="font-size: 10px;">Max 2MB & file should be DOC or DOCX</span>)</label>

                            <div class="flex gap-[10px]">
                                {{-- Show Old File Name --}}
                                <input type="text"
                                    id="uploaded_para_word"
                                    name="uploaded_para_word"
                                    placeholder="Upload Word File"
                                    class="uploaded_para_word"
                                    readonly
                                    value="{{ $auditPara->word_file_path ?? '' }}">

                                <label class="upload_cust mb-0 hover-effect-btn"> Upload File
                                    <input type="file" id="upload_para_word" name="upload_para_word" class="hidden upload_para_word">
                                    <input type="hidden" id="para_word_file" name="para_word_file"
                                        value="{{ $auditPara->word_file_path ?? '' }}">
                                </label>
                            </div>

                            {{-- View Existing Word File --}}
                            @if (!empty($auditPara->word_file_path))
                            @php
                            $filePath = 'uploads/audit-management/para/';
                            $fileName = basename($auditPara->word_file_path);
                            $fileUrl = url('audit-management/view/files') .
                            '?pathName=' . $filePath .
                            '&fileName=' . $fileName;
                            @endphp

                            <a href="{{ $fileUrl }}" target="_blank" class="text-primary underline" title="View Word file">
                                View File
                            </a>
                            @endif

                            @error('upload_para_word')
                            <div class="error-message">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="">
                            ...
                            <select class="" name="office">
                                <option value="">Select Office</option>
                                @foreach ($officeTypes as $office)
                                <option value="{{ $office->id }}"
                                    {{ $auditPara->ref_office_id == $office->id ? 'selected' : '' }}>
                        ...
                        <div class="">
                            ...
                            <select class="" name="department">
                                ...
                                <option value="{{ $department->id }}"
                                    {{ $auditPara->ref_department_id == $department->id ? 'selected' : '' }}>
                                    {{ $department->name }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="">
                            ...
                            <select class="" name="assign_to" id="assign_to">
                                <option value="">Select Candidate</option>


                                @if(isset($auditPara->assign_to) && $auditPara->assignTo)
                                <option value="{{ $auditPara->assign_to }}" selected>
                                    {{ $auditPara->assignTo->name }} ({{ $auditPara->assignTo->email }})
                                </option>
                                @endif
                            </select>
                        </div>
                    ...
                    <div class="button_flex_cust_form">
                        <button class="hover-effect-btn fill_btn" type="submit">Update Para</button>
                    </div>
...
<script src="{{ asset('/public/validation/auditPara.js') }}"></script>
<script></script>
@endpush
...
@section('dashboard_contents')
                            ...
                            <div class="text-blue-600 hover:underline cursor-pointer"><a href="{{ url('/public/uploads/bg/' . $bg->mode_of_confirmation_file) }}" target="_blank" class="text-blue-600 hover:underline cursor-pointer">{{ $bg->mode_of_confirmation_file }}</a></div>
                                ...
                                @if ($renew->status == 'P')
                                    <div class="flex gap-4 mt-4">
                                        {{-- <button type="button" onclick="openRemarkModal({{ $renew->id }}, 'Accepted')"
                                            class="bg-purple-600 text-green px-3 py-1 rounded">
                                        ...
                                        {{-- <button type="button" onclick="openRemarkModal({{ $renew->id }}, 'Rejected')"
                                            class="bg-red-600 text-red px-3 py-1 rounded">
                                            <i class="fa fa-xmark me-2"></i> Reject
                                        </button> --}}
    ...
    {{-- <div id="remarkModal" class="fixed inset-0 bg-black hidden items-center justify-center z-50">
        <div class="bg-white w-full max-w-md rounded-4xl shadow-xl p-6">
            <h2 class="text-xl font-semibold text-gray-800 mb-6">Enter Remark</h2>

            <form method="POST" action="{{ route('bgms.verifier.renew.update') }}">
                @csrf

                <input type="hidden" name="id" id="modal_renew_id">
                <input type="hidden" name="action" id="modal_action">
                <div class="mb-4">
                    <label for="modal_remark" class="block text-sm font-medium text-gray-700 mb-1">Remark</label>
                    <textarea name="remarks" id="modal_remark" rows="3"
                ...
                <div class="flex justify-end space-x-2 mt-4">
                    <button type="button" onclick="closeRemarkModal()"
                        class="px-4 py-2 rounded-lg text-sm bg-green-500 hover:bg-gray-600 text-black">
                        Cancel
                    </button>
                    <button type="submit" class="px-4 py-2 rounded-lg text-sm bg-blue-600 hover:bg-blue-700 text-white">
                        Submit
                    </button>
                </div>
            </form>

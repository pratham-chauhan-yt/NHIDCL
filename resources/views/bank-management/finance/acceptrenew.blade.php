...
@section('dashboard_contents')
  ...
  // dd($bg->bg_no);
                    ...
                    @if ($bg->renewals)
                        @foreach ($bg->renewals as $renew)
                            <div class="bg-white rounded-md shadow-md p-6">
                                ...
                                <div class="mt-2 grid grid-cols-2 gap-y-3">
                                    <div><strong class="text-gray-600">Issue Date:</strong></div>
                                    <div class="text-gray-700">{{ format_datetime($renew->issue_date) }}</div>
                                    <div><strong class="text-gray-600">Valid Upto:</strong></div>
                                    <div class="text-gray-700">{{ format_datetime($renew->valid_upto) }}</div>
                                    <div><strong class="text-gray-600">Claim Expiry Date:</strong></div>
                                    <div class="text-gray-700">{{ format_datetime($renew->claim_expiry_date) }}</div>
                                    ...
                                    @php
                                        $label = '';
                                        $statusColor = '';
                                        $statusText = getBgStatus($renew->status);
                                        $dateLabel = '';
                            ...
                            <div class="text-blue-600 hover:underline cursor-pointer"><a href="{{ url('/public/uploads/bg/' . $bg->mode_of_confirmation_file) }}" target="_blank" class="text-blue-600 hover:underline cursor-pointer">{{ $bg->mode_of_confirmation_file }}</a></div>
                                ...
                                @if ($renew->status == 'A')
                                    <div class="flex gap-4 mt-4">
                                        ...
                                        <form method="POST"
                                            action="{{ route('bgms.verifier.verify.renew', encryptId($renew->id)) }}"
                                            style="display:inline;">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit"
                                                onclick="return confirm('Are you sure want to verify?')"
                                                class="btn btn-sm btn-info mark-as-completed-btn">
                                                Verify
                                            </button>
                                        ...
                                        <button type="button" class="btn btn-sm btn-danger referback-btn"
                                            data-action="{{ route('bgms.verifier.referback.renew', encryptId($renew->id)) }}">
                                            Refer Back
                                        </button>
                                    </div>
                                @endif
                                ...
                                <button type="button" class="btn btn-sm btn-primary receive-btn" id="receive-btn">Receive</button>
                                <button type="submit" class="btn btn-sm btn-danger referback-btn">Refer Back</button>
                                </div> -->
                            </div>
                        @endforeach
                    @endif
    ...
    <form class="form_grid_cust" id="bg-form" method="POST" action="{{ route('bgms.finance.update.financeacceptrenew', encryptId($bg->id)) }}">
    @csrf
    <h2>Accept BG</h2>
    <div class="inpus_cust_cs form_grid_dashboard_cust_" style="display:block;">
        <div>
            <label class="required-label" for="remarks">Physical Location</label>
        ...
        @php
        $renew = DB::table('nhidcl_bgm_renew_bg')->where('nhidcl_bgm_bank_guarantees_id',$bg->id)->where('status','Renew Receive')->first();
        @endphp
        @if($renew)
        ...
        <div id="bg-reject" class="custom-modal" style="display:none;">
        <form class="form_grid_cust" id="bg-form" method="POST">
            @csrf
            @method('put')
            <h2>Refer Back Renew</h2>
            <div class="inpus_cust_cs form_grid_dashboard_cust_" style="display:block;">
                <div class="">
                ...
                <input type="hidden" name="nhidcl_bgm_bank_guarantees_id" value="{{ $bg->id }}">
                <div class="modal-buttons">
                    @if($bg->status == 'Renew Referback')
                    <button type="button" class="btn btn-cancel" id="closeReferModal">Cancel</button>
                    <button type="submit" class="btn btn-primary">Submit</button>
                    @endif
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const referBackBtn = document.querySelector(".referback-btn");

    const modalReceive = document.getElementById("bg-modal");
    const modalReject = document.getElementById("bg-reject");

    receiveBtn.addEventListener("click", function (e) {
        e.preventDefault(); // prevent form submit
        modalReceive.style.display = "block";
    });

    referBackBtn.addEventListener("click", function (e) {
        e.preventDefault(); 
    ...
    document.querySelectorAll("#closeModal").forEach(btn => {
        btn.addEventListener("click", function () {
            modalReceive.style.display = "none";
            modalReject.style.display = "none";
        });
    });

    window.addEventListener("click", function (e) {
        if (e.target === modalReceive) modalReceive.style.display = "none";
        if (e.target === modalReject) modalReject.style.display = "none";
    });
});
</script>

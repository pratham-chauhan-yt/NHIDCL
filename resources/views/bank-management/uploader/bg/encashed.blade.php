...
<div class="container-fluid md:p-0">
    <div class="top_heading_dash__">
        <div class="main_hed">Encashed BG</div>
    </div>
</div>
        ...
        <div id="project" class="tabcontent">
            <form id="bg-search-form" class="flex flex-wrap items-center gap-2">
                ...
                <div class="flex w-full">
                    <h2>Project Name</h2>
                    <select name="ref_name" id="project_select" class="form-control w-full">
            ...
            <div class="table_over">
                <table class="cust_table__ table_sparated" id="encashed-bg-table">
                    <thead class="">
                        <tr>
                            <th>S. No.</th>
                            <th>BG Id</th>
...
<script>
    ...
    $('#bg-received-form').on('submit', function(e) {
        ...
        let wordCount = remarks.split(/\s+/).filter(function(word) {
    ...
    $('#searchButton').on('click', function() {
        ...
        $.ajax({
            url: "{{route('bgms.project.encashedSearch')}}",
            type: "POST",
            data: {
                state: state,
                _token: "{{ csrf_token() }}"

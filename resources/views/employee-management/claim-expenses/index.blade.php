  ...
  <div class="inner_page_dash__">
    <div class="my-4">
      ...
      <div id="Home" class="tabcontent" style="display: block;">
        <form action="{{ route('employee-management.claim.expenses.store') }}" method="post" id="hrAssignAssetForms" class="form_grid_cust">
          @csrf
          <div class="inpus_cust_cs form_grid_dashboard_cust_">
            <div class="form-input">
              <label class="required-label">Purpose</label>
              <input type="text" name="purpose" id="purpose" value="{{ old('purpose') }}" placeholder="Purpose" data-validate="required" data-error="Please enter claim purpose.">
            </div>
            <div class="form-input">
              <label class="required-label">Amount</label>
              <input type="text" name="amount" id="amount" value="{{ old('amount') }}" placeholder="Enter claim amount" data-validate="required" data-error="Please enter claim amount.">
            </div>
            <div class="form-input">
              <label class="required-label">From/To Date</label>
              <div class="grid grid-cols-2 gap-[10px]">
                <input type="date" name="from_date" id="from_date" max="{{ date('Y-m-d') }}" value="{{ old('from_date') }}" data-validate="required" data-error="Please choose from date.">
                <input type="date" name="to_date" id="to_date" min="{{ date('Y-m-d') }}" value="{{ old('to_date') }}" data-validate="required" data-error="Please choose to date.">
              </div>
            </div>
            <div class="form-input attachment_payment_proof">
              <label class="required-label">Payment Proof</label>
              <div class="flex gap-[10px]">
                <input id="payment_proof_txt" name="payment_proof_txt" type="text" class="payment_proof_txt" placeholder="Upload payment proof" readonly>
                ...
                <input type="hidden" name="payment_proof" id="payment_proof" value="{{ old('payment_proof') }}">
              </div>
            ...
            <div class="form-input">
              ...
              <textarea name="description" id="description" rows="3" placeholder="Brief Description" data-validate="required" data-error="Please enter claim brief description.">{{ old('description') }}</textarea>
              <small class="form-text text-muted text-red">
          ...
          <div class="button_flex_cust_form">
            <button class="hover-effect-btn fill_btn" type="submit" name="submit" value="Submit" id="savehrAssignAsset">
              Submit
            </button>
          </div>
        </form>

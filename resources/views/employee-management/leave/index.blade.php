    ...
    <div class="inner_page_dash__">
        <div class="my-4">
            ...
            <div id="Home" class="tabcontent" style="display: block;">
                <form action="{{ route('employee-management.apply.leave.store') }}" method="post" class="form_grid_cust" id="FormValidations">
                    @csrf
                    <div class="inpus_cust_cs form_grid_dashboard_cust_">
                        <div class="form-input">
                            <label class="required-label">Purpose Of Leave</label>
                            <input type="text" name="purpose_of_leave" id="purpose_of_leave" value="{{ old('purpose_of_leave') }}" placeholder="Enter purpose of leave" data-validate="required" data-error="Please enter purpose of leave.">
                        </div>
                        <div class="form-input">
                            <label class="required-label">From/To Date</label>
                            <div class="grid grid-cols-2 gap-[10px]">
                                <input type="date" name="from_date" id="from_date" value="{{ old('from_date') }}" max="{{ date('Y-m-d') }}" data-validate="required" data-error="Please choose from date.">
                                <input type="date" name="to_date" id="to_date" value="{{ old('to_date') }}" min="{{ date('Y-m-d') }}" data-validate="required" data-error="Please choose to date.">
                            </div>
                        </div>
                        <div class="form-input">
                            <label class="required-label">Prefix/Suffix Date</label>
                            <div class="grid grid-cols-2 gap-[10px]">
                                <input type="date" name="prefix_from" id="prefix_from" value="{{ old('prefix_from') }}" max="{{ date('Y-m-d') }}" data-validate="required" data-error="Please choose prefix from date.">
                                <input type="date" name="prefix_to" id="prefix_to" value="{{ old('prefix_to') }}" min="{{ date('Y-m-d') }}" data-validate="required" data-error="Please choose prefix to date.">
                            </div>
                        </div>
                        <div class="form-input">
                            <label class="required-label">Checker</label>
                            <select name="ref_checker_id" id="ref_checker_id" data-validate="required" data-error="Please choose your manager.">                               <option value="">---- Choose your manager ----</option>
                        ...
                        <div class="form-input">
                            ...
                            <textarea name="address_during_leave_period" id="address_during_leave_period" rows="3" placeholder="Address during the leave period" data-validate="required" data-error="Please enter address during the leave period.">{{ old('address_during_leave_period') }}</textarea>

...
@section('dashboard_content')
    <div class="container-fluid md:p-0">
        <div class="top_heading_dash__">
            <div class="main_hed">Approve Attendance</div>
        </div>
    </div>
                    ...
                    <div class="inpus_cust_cs form_grid_dashboard_cust_">
                        <div class="form-input">
                            ...
                            <select name="ref_status_id" id="ref_status_id">
                                ...
                                <option value="{{ $statusdata->id }}" {{ $attendance->ref_status_id == $statusdata->id ? 'selected' : '' }}>{{ $statusdata->type }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-input">
                            <label class="required-label">Remark</label>
                            @if(auth()->user()->id == $attendance->ref_approver_id)
                            ...
                            @endif
                            @if(auth()->user()->id == $attendance->ref_checker_id)
